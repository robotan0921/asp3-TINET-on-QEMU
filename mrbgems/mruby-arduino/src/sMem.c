...
char	*str;
	...
	int ret = 0;
	if( num==0 ){
		EEP_fwrite(Fp0, str, &len);
		ret = len;
	}
	else if( num==1 ){
		EEP_fwrite(Fp1, str, &len);
		ret = len;
	}
...
char	*str;
	...
	if( num==0 ){
		if( mode==1 ){
			if(EEP_fopen(Fp0, str, EEP_APPEND) != -1){
				ret = num;
			}
		}
		else if( mode==2 ){
			if(EEP_fopen(Fp0, str, EEP_WRITE) != -1){
				ret = num;
			}
		}
		else{
			if(EEP_fopen(Fp0, str, EEP_READ) != -1){
				ret = num;
			}
		}
	}
		...
		if( mode==1 ){
			if(EEP_fopen(Fp1, str, EEP_APPEND) != -1){
				ret = num;
			}
		}
		else if( mode==2 ){
			if(EEP_fopen(Fp1, str, EEP_WRITE) != -1){
				ret = num;
			}
		}
		else{
			if(EEP_fopen(Fp1, str, EEP_READ) != -1){
				ret = num;
			}
		}
	}
...
int ret = 0;
	...
	if( num==0 ){
		if( size==-1 ){
			if( EEP_fseek(Fp0, 0, EEP_SEEKEND) ){
				ret = 1;
			}
		}
		else{
			if( EEP_fseek(Fp0, size, EEP_SEEKTOP) ){
				ret = 1;
			}
		}
	}
		...
		if( size==-1 ){
			if( EEP_fseek(Fp1, 0, EEP_SEEKEND) ){
				ret = 1;
			}
		}
		else{
			if( EEP_fseek(Fp1, size, EEP_SEEKTOP) ){
				ret = 1;
			}
		}
	}
...
int ret = 1;
	...
	if(mode == 1){
		if(EEP_fcopy(RSTRING_PTR(src), RSTRING_PTR(dst)) == -1){
			ret = 0;
		}
	}
	return mrb_fixnum_value( ret );
