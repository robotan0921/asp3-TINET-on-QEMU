...
mrb_value mrb_serial_begin(mrb_state *mrb, mrb_value self)
{
	...
	if (num == 0) {
		...
		if (serial[num] != 0) {
			serial_free(serial[num]);
			wait_ms(50);
			free(serial[num]);
		}

		serial[num] = calloc(1, sizeof(serial_t));
	}
	...
	if (sci != SCI_USB0) {
		PinName txpin, rxpin;
		portToPins(sci, &txpin, &rxpin);
		serial_init(serial[num], txpin, rxpin);
		serial_baud(serial[num], bps);
	}
...
{
	if (num < 10) {
		serial_print(serial[num], RSTRING_PTR(text));
	}
	else if (num < 20) {
		serial_println(serial[num - 10], RSTRING_PTR(text));
	}
	else {
		serial_println(serial[num - 20], "");
	}
}
...
mrb_value mrb_serial_read(mrb_state *mrb, mrb_value self)
{
	...
	if (num >= 0 && num < SERIAL_MAX) {
		if (serial_readable(serial[num])) {
			return mrb_fixnum_value(serial_getc(serial[num]));
		}
	}
...
mrb_value mrb_serial_write(mrb_state *mrb, mrb_value self)
{
	...
	return mrb_fixnum_value(serial_write(serial[num], (const unsigned char *)str, len));
}
...
mrb_value mrb_serial_available(mrb_state *mrb, mrb_value self)
{
	...
	return mrb_fixnum_value(serial_readable(serial[num]));
}
