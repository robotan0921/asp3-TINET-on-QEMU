...
mrb_value mrb_pancake_Clear(mrb_state *mrb, mrb_value self)
{
	...
	wait_ms(PANCAKE_WAIT);
	serial_write(serial[SerialNum], (const unsigned char *)PanSend, PanSend[1]);
...
mrb_value mrb_pancake_Serial(mrb_state *mrb, mrb_value self)
{
	...
	if (serial[SerialNum] != 0) {
		serial_free(serial[SerialNum]);
		wait_ms(50);
	}
	serial[SerialNum] = calloc(1, sizeof(serial_t));
	portToPins(SCI_SCI0P2x, &txpin, &rxpin);
	serial_init(serial[SerialNum], txpin, rxpin);
	serial_baud(serial[SerialNum], PANCAKE_BAURATE);
