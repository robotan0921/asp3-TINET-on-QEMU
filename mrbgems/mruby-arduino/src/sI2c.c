...
mrb_value mrb_i2c_write(mrb_state *mrb, mrb_value self)
{
	...
	i2c_start(&Wire);
	i2c_byte_write(&Wire, deviceID);
	i2c_byte_write(&Wire, addr);
	i2c_byte_write(&Wire, dat);

	return mrb_fixnum_value(i2c_stop(&Wire));
}
...
mrb_value mrb_i2c_read(mrb_state *mrb, mrb_value self)
{
	...
	i2c_start(&Wire);
	i2c_byte_write(&Wire, deviceID);
	i2c_byte_write(&Wire, addrL);
	i2c_stop(&Wire);

	i2c_start(&Wire);
	i2c_byte_write(&Wire, deviceID | 1);;
	dat = i2c_byte_read(&Wire, 1);
	i2c_stop(&Wire);

	if (n >= 3) {
		i2c_start(&Wire);
		i2c_byte_write(&Wire, deviceID);
		i2c_byte_write(&Wire, addrH);
		i2c_stop(&Wire);

		i2c_start(&Wire);
		i2c_byte_write(&Wire, deviceID | 1);;
		datH = i2c_byte_read(&Wire, 1);
		i2c_stop(&Wire);
...
mrb_value mrb_i2c_lwrite(mrb_state *mrb, mrb_value self)
{
	...
	return mrb_fixnum_value(i2c_byte_write(&Wire, dat));
}
...
mrb_value mrb_i2c_requestFrom(mrb_state *mrb, mrb_value self)
{
	...
	i2c_read(&Wire, addr, rxdata, cnt, 1);
...
{
	return mrb_fixnum_value(i2c_byte_read(&Wire, 1));
}
