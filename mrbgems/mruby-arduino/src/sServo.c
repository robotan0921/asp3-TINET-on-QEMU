...
struct pin_node *servo[ATTACH_MAX];
...
mrb_value mrb_servo_attach(mrb_state *mrb, mrb_value self)
{
	...
	if (servo[ch] != 0) {
		pwmout_free(&servo[ch]->pwmout);
		/* servo[ch] = 0; */
	}

	servo[ch] = wrb2sakura(pintype_pwmout, pin);
	servo[ch]->min = min;
	servo[ch]->max = max;
...
mrb_value mrb_servo_write(mrb_state *mrb, mrb_value self)
{
	...
	DEBUG_PRINT("pwmout_t", angle);

	min = servo[ch]->min;
	max = servo[ch]->max;
	angle *= servo[ch]->period;

	pwmout_pulsewidth_us(&servo[ch]->pwmout, (angle * (max - min)) / 180 + min);
...
mrb_value mrb_servo_us(mrb_state *mrb, mrb_value self)
{
	...
	pwmout_pulsewidth_us(&servo[ch]->pwmout, us);
...
mrb_value mrb_servo_attached(mrb_state *mrb, mrb_value self)
{
	...
	if (get_pin_type(servo[ch]->pin) == pintype_pwmout) {
		ret = 1;
	}
...
void servo_Init(mrb_state *mrb)
{
	...
	struct RClass *servoModule = mrb_define_module(mrb, "pwmout_t");
