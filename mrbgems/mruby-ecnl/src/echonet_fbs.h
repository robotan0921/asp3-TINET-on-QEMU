...
/* T_EDATA管理領域サイズ */
#define DEF_ECN_EDT_HDR_LEN (sizeof(intptr_t))
...
typedef struct
{
	...
	union{
		...
		struct ecn_fbs_queue *mbxid;	/* 送信元メールボックスID（内部メッセージ） */
	}sender;
	union{
		ECN_ENOD_ID id;			/* 送信先ノードID（ECHONET電文） */
		struct ecn_fbs_queue *mbxid;	/* 送信先メールボックスID（内部メッセージ） */
	}target;
	union{
		ECN_ENOD_ID id;			/* 返信先ノードID（ECHONET電文） */
		struct ecn_fbs_queue *mbxid;	/* 返信先メールボックスID（内部メッセージ） */
	}reply;
 ...
 *		ER				0:ok, 非0:NG
 */
ER _ecn_fbs_get_data(mrb_state *mrb, ECN_FBS_ID fa_id, void *fa_buf, ECN_FBS_SSIZE_T fa_maxlen,
