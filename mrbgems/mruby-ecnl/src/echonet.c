 ...
 */
ER ecn_ref_eobj(ecnl_svc_task_t *svc, ID fa_eobjid, T_REOBJ *fp_eobj)
{
	...
	if ((fa_eobjid <= 0) || (fa_eobjid > svc->tmax_eobjid))
		return E_OBJ;	/* オブジェクト未登録 */

	p_obj = echonet_svctask_get_eobjinib(svc, fa_eobjid);
 ...
 */
ER ecn_ref_eprp(ecnl_svc_task_t *svc, ID fa_eobjid, uint8_t fa_epc, T_RPRP *fp_eprp)
{
	const EOBJINIB *p_obj;
	const EPRPINIB **p;
	uint_t i;
	...
	p_obj = svc->eobjinib_table[fa_eobjid - 1];
 ...
 */
ER ecn_esv_seti(ecnl_svc_task_t *svc, T_EDATA **ppk_esv, ID fa_deoj, uint8_t fa_epc, uint8_t fa_pdc,
	const void *p_edt)
{
	return _ecn_tsk_mk_esv(svc, (ECN_FBS_ID *)ppk_esv, (ID)1, fa_deoj, fa_epc, fa_pdc, p_edt, ESV_SET_I);
}
 ...
 *	作成した電文の先頭アドレスはポインタ経由で返される。
 */
ER ecn_esv_setc(ecnl_svc_task_t *svc, T_EDATA **ppk_esv, ID fa_deoj, uint8_t fa_epc, uint8_t fa_pdc,
	const void *p_edt)
{
	return _ecn_tsk_mk_esv(svc, (ECN_FBS_ID *)ppk_esv, (ID)1, fa_deoj, fa_epc, fa_pdc, p_edt, ESV_SET_C);
}
 ...
 * 作成した電文の先頭アドレスはポインタ経由で返される。
 */
ER ecn_esv_get(ecnl_svc_task_t *svc, T_EDATA **ppk_esv, ID fa_deoj, uint8_t fa_epc)
{
	return _ecn_tsk_mk_esv(svc, (ECN_FBS_ID *)ppk_esv, (ID)1, fa_deoj, fa_epc, 0, NULL, ESV_GET);
}
 ...
 * 作成した電文の先頭アドレスはポインタ経由で返される。
 */
ER ecn_esv_inf_req(ecnl_svc_task_t *svc, T_EDATA **ppk_esv, ID fa_deoj, uint8_t fa_epc)
{
	return _ecn_tsk_mk_esv(svc, (ECN_FBS_ID *)ppk_esv, (ID)1, fa_deoj, fa_epc, 0, NULL, ESV_INF_REQ);
}
 ...
 * 作成した電文の先頭アドレスはポインタ経由で返される。
 */
ER ecn_esv_set_get(ecnl_svc_task_t *svc, T_EDATA **ppk_esv, ID fa_deoj, uint8_t fa_epc, uint8_t fa_pdc,
	const void *p_edt)
{
	return _ecn_tsk_mk_esv(svc, (ECN_FBS_ID *)ppk_esv, (ID)1, fa_deoj, fa_epc, fa_pdc, p_edt, ESV_SET_GET);
}
 ...
 * 読み出しプロパティ配列に移る時に用いる。
 */
ER ecn_trn_set_get(mrb_state *mrb, T_EDATA *ppk_esv, int *p_trn_pos)
{
	ER	a_ret;
	...
	*p_trn_pos = _ecn_fbs_get_datalen(*(ECN_FBS_ID *)&ppk_esv);
	...
	a_ret = _ecn_fbs_add_data_ex(mrb, *(ECN_FBS_ID *)&ppk_esv, &ppk_esv->hdr.edata.opc, 1);
	if (a_ret) {
 ...
 */
ER ecn_end_set_get(mrb_state *mrb, T_EDATA *ppk_esv, int fa_trn_pos)
{
	ER		a_ret;
	int		a_wr_opc;
	uint8_t	a_rd_opc;
	...
	a_wr_opc = _ecn_fbs_peek(mrb, *(ECN_FBS_ID *)&ppk_esv, fa_trn_pos);
	if (a_wr_opc < 0) {
	...
	a_ret = _ecn_fbs_poke(mrb, *(ECN_FBS_ID *)&ppk_esv, fa_trn_pos, a_rd_opc);
	if (a_ret) {
 ...
 */
ER ecn_esv_infc(ecnl_svc_task_t *svc, T_EDATA **ppk_esv, ID fa_deoj, ID fa_seoj, uint8_t fa_sepc)
{
	return _ecn_tsk_mk_esv(svc, (ECN_FBS_ID *)ppk_esv, fa_seoj, fa_deoj, fa_sepc, 0, NULL, ESV_INFC);
}
...
{
	return ecn_add_edt(mrb, pk_esv, epc, 0, 0);
}

/*
 * 要求電文へのプロパティデータ追加 (プロパティおよび付随データを追加する)
 */
ER ecn_add_edt(mrb_state *mrb, T_EDATA *pk_esv, uint8_t fa_epc, uint8_t fa_pdc, const void *p_edt)
{
	ECN_FBS_ID	a_fbs_id;
	ER			a_ret;
	T_ECN_PRP	a_ecn_prp;
	...
	a_ret = _ecn_fbs_add_data_ex(mrb, a_fbs_id, &a_ecn_prp, sizeof(a_ecn_prp));
	if (a_ret) {
		ECN_DBG_PUT_4("ecn_add_edt(): _ecn_fbs_add_data_ex(*, ecn_prp{epc:0x%02X}, %u) result = %d:%s",
			a_ecn_prp.epc, sizeof(a_ecn_prp), a_ret, itron_strerror(a_ret));
		goto lb_except;
	}
	if (0 < fa_pdc) {
		/* 付随データ追加 */
		...
		if (a_ret) {
			ECN_DBG_PUT_4("ecn_add_edt(): _ecn_fbs_add_data_ex(*, ecn_prp{epc:0x%02X} edt, %u) result = %d:%s",
				a_ecn_prp.epc, fa_pdc, a_ret, itron_strerror(a_ret));
			goto lb_except;
		}
	}
	/* プロパティ数インクリメント */
...
ER ecn_prcv_esv(ecnl_svc_task_t *svc, T_EDATA **ppk_esv)
{
	...
	a_ret = ecn_fbs_dequeue(&svc->api_mbxid, &p_msg);
	if (a_ret != E_OK) {
		*ppk_esv = NULL;
		return a_ret;
	}
	...
	*ppk_esv = (T_EDATA *)p_msg;
	return ((p_msg)->hdr.type == ECN_MSG_ECHONET) ? E_OK : E_BRK;
}
...
ID ecn_get_enod(ecnl_svc_task_t *svc, T_EDATA *pk_esv)
{
	...
	p_snod = _ecn_eno_fnd(svc, ((T_ECN_FST_BLK *)pk_esv)->hdr.sender.id);
	if (p_snod != NULL) {
...
#endif
	return echonet_svctask_get_eobjid(svc, p_sobj);
}
...
ER ecn_itr_nxt(mrb_state *mrb, T_ENUM_EPC *pk_itr, uint8_t *p_epc, uint8_t *p_pdc, void *p_edt)
{
	...
	ECN_CAP_PUT_2("ecn_itr_nxt() rd.cur=b%dp%d", pk_itr->cur.blk, pk_itr->cur.pos ECN_CAP_PUT_2(;
#endif
	if (pk_itr->count <= pk_itr->got_ct && 0 < pk_itr->next_blk_ct) {
		...
		/* 次ブロックのプロパティ数を読み取る */
		a_rd_len = 0;
		a_ret = _ecn_fbs_get_data(mrb, a_fbs_id, &a_ecn_prp.pdc, 1, &a_rd_len);
		if (a_ret != E_OK)
			goto lb_except;
		if (0 < a_rd_len) {
	...
	memset(&a_ecn_prp, 0, sizeof(a_ecn_prp));
	a_rd_len = 0;
	a_ret = _ecn_fbs_get_data(mrb, a_fbs_id, &a_ecn_prp, sizeof(a_ecn_prp), &a_rd_len);
	if (a_ret != E_OK)
		goto lb_except;
...
#ifdef ECN_ENA_ITR_NXT_CARP
	ECN_CAP_PUT_2("ecn_itr_nxt() read: ct=%d/%d", pk_itr->got_ct, pk_itr->count ECN_CAP_PUT_2(;
#endif
 ...
 * 応答電文待ちの割り込み送信
 */
ER ecn_brk_wai(ecnl_svc_task_t *svc, const void *p_dat, int fa_datsz)
{
	ER				a_ret;
	ECN_FBS_ID		a_fbs_id = { 0 };	/* 要求電文用メモリ	*/

	/* 応答電文待ちの割り込み電文作成 */
	a_ret = _ecn_mk_brk_wai(svc, &a_fbs_id, p_dat, fa_datsz);
	if (a_ret)
		return a_ret;

	/* 割り込み送信 */
	a_ret = _ecn_tsk_snd_mbx(svc, a_fbs_id, true);
	if (a_ret) {
		ECN_DBG_PUT_2("_ecn_tsk_snd_mbx() result = %d:%s",
 ...
 * 割り込みデータの取得
 */
ER ecn_get_brk_dat(mrb_state *mrb, T_EDATA *pk_esv, void *p_buf, int fa_bufsz, int *p_datsz)
{
	ECN_FBS_ID	a_fbs;
	ER			a_ret;
	...
	if (p_buf && 0 < fa_bufsz) {
		a_len = 0;
		a_ret = _ecn_fbs_get_data(mrb, a_fbs, p_buf, fa_bufsz, &a_len);
		if (a_ret || !a_len) {
